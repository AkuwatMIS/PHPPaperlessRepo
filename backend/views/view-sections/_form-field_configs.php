            ...
            <tr class="room-item">
                <td class="vcenter">
                    ...
                    if (! $modelFieldsConfig->isNewRecord) {
                        echo Html::activeHiddenInput($modelFieldsConfig, "[{$indexField}][{$indexFieldsConfig}]id");
                    }
                    ?>
                    <?= $form->field($modelFieldsConfig, "[{$indexField}][{$indexFieldsConfig}]key_name")->label(false)->textInput(['maxlength' => true]) ?>
                </td>
                <td>  <?= $form->field($modelFieldsConfig, "[{$indexField}][{$indexFieldsConfig}]value")->label(false)->textInput(['maxlength' => true]) ?> </td>
                <td> <?php echo $form->field($modelFieldsConfig, "[{$indexField}][{$indexFieldsConfig}]parent_id")->label(false)->dropDownList($list,['data-config_index'=> '0','class'=> 'form-control update-keys index-config-0' ,'maxlength' => true])
