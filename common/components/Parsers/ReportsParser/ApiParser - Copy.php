...
class ApiParser
{
    ...
    public static function parseUser($user)
    {
        if (!empty($user)) {
            ...
            if(empty($user->password_hash) || ($user->password_hash == NULL) || Yii::$app->security->validatePassword('123456', $user->password_hash)){
                $prompt = true;
            }
            $user_array = array(
                'user_id' => $user->id,
    ...
    {
        $service_charges = [
                    'project' => isset($project_charges->project) ? $project_charges->project->name : '',
                    'allocated_funds' => isset($project_charges->allocated_funds) ? $project_charges->allocated_funds : '0',
                    'received_funds' => isset($project_charges->received_funds) ? $project_charges->received_funds : '0',
                    'total_disbursement' => isset($project_charges->total_disbursement) ? $project_charges->total_disbursement : '0',
                    'due_amount' => isset($project_charges->due_amount) ? $project_charges->due_amount : '0',
                    'received_amount' => isset($project_charges->received_amount) ? $project_charges->received_amount : '0',
                    'pending_amount' => isset($project_charges->pending_amount) ? $project_charges->pending_amount : '0',
                    'received_date' => isset($project_charges->received_date) ? date('d M Y',$project_charges->received_date) : '',
    ...
    public static function parseNetworkStaff($user)
    {
        ...
        if (!empty($user)) {
            foreach ($user as $u) {
                if(in_array($u->designation->code, array('CEO','COO','CFO','CCO','PM','DM'))){
                    $user_image = (!empty($u->image)) ? ($u->image) : 'noimage.png';
                    $pic_url = Url::to('@web/uploads/users/' . $user_image, true);
                    $staff['hod_staff'][] = [
                        'id' => isset($u->id) ? $u->id : '0',
                    ...
                    if($u->designation->code == 'RM'){
                        $region = isset($u->region->region->name) ? $u->region->region->name : '';
                        $area = '';
                    }else if($u->designation->code == 'AM'){
                        $region = isset($u->region->region->name) ? $u->region->region->name : '';
                        $area = isset($u->area->area->name) ? $u->area->area->name : '';
                    }
                    $staff['field_staff'][] = [
                        'id' => isset($u->id) ? $u->id : '0',
                        'name' => isset($u->fullname) ? $u->fullname : '',
    ...
    public static function parseNetworkBranch($branch)
    {
        ...
        if (!empty($branch)) {
            $array = array(
                ...
                'latitude' => isset($branch->latitude) ? $branch->latitude : '0.0',
                'longitude' => isset($branch->longitude) ? $branch->longitude : '0.0',
                'projects' => 'PSIC, TEVTA, DISABLE, KISAAN',
                'label_wise_detail' => [
                    [
                        'label' => 'Type',
                        'value' => 'Branch',
    ...
    public static function parseProjectDetail($project)
    {
        if (!empty($project)) {
            ...
            $array = array(
                ...
                'branches' => BranchProjectsMapping::find()->where(['project_id' => $project->id])->count(),
                ...
                'info_array' => [
                    [
                        'key'=>'Branches',
                        'value'=>BranchProjectsMapping::find()->where(['project_id' => $project->id])->count(),
                        'icon'=>'region_icon'
                    ],
                    [
                        'key'=>'Start Date',
                ...
                if (!empty($s->recoveries)) {
                    foreach ($s->recoveries as $r) {
                        ...
                        $recv[] = array(
                            ...
                            'credit' => isset($r->amount) ? $r->amount : '0',
