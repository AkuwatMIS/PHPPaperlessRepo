...
class TasdeeqHelper
{
    public static function getReport($url, $body, $auth_token)
    {
        ...
        $response = NetworkHelper::curlCall($url . '/TestCreditInformationReport/', $headers, $body, true);
    ...
    public static function login($url, $cred_model)
    {
        ...
        $response = NetworkHelper::curlCall($url . '/Authenticate/', $headers, $body, true);
        print_r($response);
        die();
        if ($response->statusCode == '111') {
    ...
    public static function parseRequetBody($application)
    {
        $arr['reportDataObj'] = [
            ...
            "loanAmount" => isset($application->req_amount) ? $application->req_amount : '0',
            "gender" => isset($application->member->gender) ? strtoupper($application->member->gender) : '',
