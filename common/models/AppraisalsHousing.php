...
class AppraisalsHousing extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['application_id', 'property_type', 'ownership', 'land_area', 'residential_area', 'purchase_price', 'address'/*, 'estimated_figures', 'estimated_start_date', 'estimated_completion_time'*/], 'required'],
            [['application_id', 'living_duration', 'no_of_rooms', 'no_of_kitchens', 'no_of_toilets', 'is_lock', 'approved_by', 'approved_on', 'assigned_to', 'created_by', 'updated_by', 'created_at', 'updated_at', 'deleted', 'platform'], 'integer'],
            [['land_area', 'residential_area', 'purchase_price', 'current_price', 'latitude', 'longitude', 'bm_verify_latitude', 'bm_verify_longitude'], 'number'],
            [['address', 'housing_appraisal_address', 'description'], 'string'],
    ...
    public function set_values($request)
    {
        ...
        if($this->land_area>20){
            $this->addError('land_area','Land Area can not be greater than 20 Marlas');
        }
        if($this->application->project_id == 132 && $this->residential_area > 10){
            $this->addError('residential_area','Covered Area can not be greater than 10 Marlas');
        }
        if($this->application->project_id != 132 && $this->residential_area > 5){
            $this->addError('residential_area','Covered Area can not be greater than 5 Marlas');
        }
        if($this->residential_area > $this->land_area){
            $this->addError('residential_area','Covered Area can not be greater than Land Area');
        }
        /*if(!empty($request ['AppraisalsHousing']['sub-from'])){
            $this->estimated_figures=json_encode(array_values($request['AppraisalsHousing']['sub-from']));
        }*/
    } 
}
