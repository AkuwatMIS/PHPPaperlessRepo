...
class AppraisalsSocial extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['platform','application_id', 'total_family_members', 'no_of_earning_hands', 'ladies', 'gents', /*'date_of_maturity',*/ 'other_loan', 'fatal_disease', 'is_lock', 'approved_by', 'approved_on', 'assigned_to', 'created_by', 'updated_by', 'created_at', 'updated_at', 'deleted','house_condition'], 'integer'],
            [['house_rent_amount','house_rent_income','job_income','business_income','other_income','expected_increase_in_income','labour_income','agriculture_income',
                'pension','through_cultivation','live_stock_income','land_size', 'total_household_income', 'utility_bills', 'educational_expenses', 'medical_expenses', 'kitchen_expenses', 'amount', 'other_expenses', 'total_expenses', 'loan_amount', 'latitude', 'longitude', 'bm_verify_latitude', 'bm_verify_longitude','income_before_corona','income_after_corona','expenses_in_corona'], 'number'],
            ...
            [['description_image','who_will_earn','earning_person_name','earning_person_cnic'], 'string', 'max' => 200],
            [['domestic_assets'], 'string'],       
    ...
    public function beforeSave($insert)
    {
        ...
        if (parent::beforeSave($insert)) {
            ...
            if(isset($this->who_will_earn) && !empty($this->who_will_earn) && ($this->who_will_earn!='self')){
                $blacklist_member = BlacklistHelper::checkBlacklist($this->earning_person_cnic);

                if(!empty($blacklist_member) && $blacklist_member!=null){
                    $application=Applications::find()->where(['id'=>$this->application_id])->one();
                    $application->status='rejected';
                    $application->reject_reason='cnic of earning person is in blacklist'.$this->earning_person_cnic."(".$blacklist_member->reason.')';
                    $application->save();
                    $this->addError('application_id','appliction is rejected.cnic of earning person is in blacklist'.$this->earning_person_cnic);
                    return false;
                }
            }
            if ($this->isNewRecord) {
    ...
    public function set_values($request)
    {
        ...
        $domestic_assets='';
        $this->load($request);
        if(isset($request['domestic_assets'])) {
            foreach ($request['domestic_assets'] as $asset => $status) {
                if ($domestic_assets != "") {
                    $domestic_assets .= "," . $asset;
                } else {
                    $domestic_assets .= $asset;
                }
            }
        }
        $this->domestic_assets=$domestic_assets;
