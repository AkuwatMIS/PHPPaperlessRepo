...
$(document).ready(function(){
$("#members-family_member_cnic").attr(\'required\', "true");
...
if(genderByCnic === 1){
   ...
   cnicGender='f';
    ...
    $(\"#membersaddress-0-address\").change(function() {
        var input = $('#membersaddress-0-address').val();
       return input.replace(/(\d){10,}/g, (match, digit) => digit.repeat(5));
    });
    
     $(\"#membersaddress-1-address\").change(function() {
        var input = $('#membersaddress-1-address').val();
       return input.replace(/(\d){10,}/g, (match, digit) => digit.repeat(5));
    });
...
<?php
$a= date('Y',strtotime('-75 year'));
$b=date('Y',strtotime('-18 year'));
...
<div class="row">
<div class="col-lg-3">
    <?= $form->field($members, "birthplace")->textInput(['placeholder'=>'Place of Birth(City Name)'])->label('Place of Birth(City Name)');  ?>
</div>
</div>
