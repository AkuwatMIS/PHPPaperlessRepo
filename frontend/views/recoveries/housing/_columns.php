...
return [
    ...
    [
        //'class'=>'\kartik\grid\DataColumn',
        'attribute' => 'charges_amount',
        'value' => function ($data) {
            return number_format($data->charges_amount);
        }
        //'pageSummary' => true,
    ],
    [
        //'class'=>'\kartik\grid\DataColumn',
        'attribute' => 'credit_tax',
        'value' => function ($data) {
            return number_format($data->credit_tax);
        }
        //'pageSummary' => true,
    ],
    [
        ...
        'buttons' => [
            'update' => function ($url, $model, $key) {
                if (!isset($model->application->loans)) {
                    return \yii\helpers\Html::a('<span class="glyphicon glyphicon-pencil"></span>', ['update', 'id' => $model->id], ['title' => 'Update', 'data-toggle' => 'tooltip']);
                }
            },
            'delete' => function ($url, $model, $key) {
                ...
                if (in_array($model->source, ['branch','1','cc']) && !isset($model->application->loans) && ($model->receive_date >= $first_of_current_month && $model->receive_date <= $last_of_current_month)) {
                    return \yii\helpers\Html::a('<span class="glyphicon glyphicon-trash"></span>', ['delete', 'id' => $model->id], ['title' => 'Delete', 'onClick' => "return confirm('Are You Sure You Want To Delete This Recovery? ')", 'data-toggle' => 'tooltip']);
                }
            },
        ],
