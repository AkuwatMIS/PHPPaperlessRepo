...
<div class="container-fluid collapse border-0" id="demo">
    ...
    <div class="row">
        ...
        <div class="col-sm-3">
            <?php
            //$regions = ArrayHelper::map(Regions::find()->asArray()->all(), 'id', 'name');
            echo $form->field($model, 'project_id')->dropDownList(\yii\helpers\ArrayHelper::map(\common\models\Projects::find()->asArray()->all(), 'id', 'name'), ['prompt' => 'Select Project'])->label('Project');
            ?>
        </div>
        <div class="col-sm-3">
            <?php
            echo $form->field($model, 'bank')->dropDownList(\common\components\Helpers\MemberHelper::getBankAccounts(), ['prompt' => 'Select Bank'])->label('Bank');
            ?>
        </div>
    </div>
