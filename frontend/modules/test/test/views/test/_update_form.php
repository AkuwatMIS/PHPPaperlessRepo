...
<div class="tables-form">
    ...
    <div class="row">
        <div class="col-md-3">
            <div class="form-group">
                ...
                <?= Html::textInput($table_name,$table_name, ['readOnly'=>true,'class' => 'form-control']) ?>
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                <?= Html::label('Update Model')?>
                <?= Html::checkbox('gen_model', false,['class' => 'checkbox', 'style'=> ['width'=> '50px','height' => '25px']]) ?>
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                <?= Html::label('Update Crud')?>
                <?= Html::checkbox('gen_crud', false,['class' => 'checkbox', 'style'=> ['width'=> '50px','height' => '25px']]) ?>
            </div>
        </div>
    </div>
    <table id="myTable" class=" table table-bordered table-stripped order-list">
        ...
        <tbody>
        <?php $i = 0;foreach ($table_attr as $attributes) { ?>
            <tr>
                <td><?= Html::textInput('att['.$i.'][col_name]', $attributes->name, ['class' => 'form-control']) ?></td>
                <td><?= Html::dropDownList('att['.$i.'][data_type]',$attributes->type,$types, ['class' => 'form-control']) ?></td>
                <td><?= Html::textInput('att['.$i.'][length]',$attributes->size, ['class' => 'form-control']) ?></td>
                <td><?= Html::dropDownList('att['.$i.'][default_value]',null,   $default, ['class' => 'form-control']) ?></td>
                <td><?= Html::checkbox('att['.$i.'][is_null]', $attributes->allowNull,['class' => 'form-control']) ?></td>
