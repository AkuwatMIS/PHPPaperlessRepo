    ...
    public function actionBmverification()
    {
        ...
        if($status == 'approved'){
            ...
            if($application->save()){
                ...
                } else if ($location_type == 'agriculture_appraisal') {
                    $appraisal = AppraisalsAgriculture::find()->where(['application_id' => $id])->one();
                }
                ...
                if($appraisal->save()){
                    $action_model = ApplicationActions::findOne(['parent_id' => $application->id, 'action' => 'approved/rejected']);
