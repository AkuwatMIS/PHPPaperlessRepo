...
class ApplicationsController extends RestController
{
    ...
    public function actionIndex()
    {
        ...
        if ($response_data['info']['totalCount'] > 0) {
            ...
            foreach ($response_data['data'] as $app_data) {
                $social_appraisal = ApplicationActions::find()->where(['parent_id' => $app_data['id'], 'action' => 'social_appraisal'])->one();
                $appraisal['appraisals_social'] =  isset($social_appraisal) ? $social_appraisal->status : 0;
                $business_appraisal = ApplicationActions::find()->where(['parent_id' => $app_data['id'], 'action' => 'business_appraisal'])->one();
                $appraisal['appraisals_business'] =isset($business_appraisal) ? $business_appraisal->status : 0;
    ...
    public function actionApplicationdetails($id)
    {
        ...
        $social_appraisal = ApplicationActions::find()->where(['parent_id' => $model->id, 'action' => 'social_appraisal'])->one();
        $app_data['social_appraisal_status'] = isset($social_appraisal) ? $social_appraisal->status : 0;
        $business_appraisal = ApplicationActions::find()->where(['parent_id' => $model->id, 'action' => 'business_appraisal'])->one();
        $app_data['business_appraisal_status'] = isset($business_appraisal) ? $business_appraisal->status : 0;
        $response = [
                ...
                if (isset($application_data) && $application_data->created_by == $user->id && in_array($application_data->branch_id,$branches)) {
                    ...
                    if (isset($model)) {
                        ...
                        try {
                            if ($flag = $model->save()) {
                                ...
                                if (isset($application['fee'])) {
                                    $operation_model = Operations::findOne(['application_id' => $model->id, 'deleted' => 0]);
                                    if (isset($operation_model)) {
                                        if ($application['fee'] == 0) {
                                            ...
                                            $operation_model->attributes = $model->attributes;
                                            $operation_model->application_id = $model->id;
                                            $operation_model->credit = $model->fee;
                                            $operation_model->operation_type_id = 1;
                                            ...
                                            if (!$flag = $operation_model->save()) {
                                                $transaction->rollBack();
                                                $error .= ApiParser::parseErrors($operation_model->errors);
                                            }
                                        }
                                    } else {
                                        if ($application['fee'] != 0) {
                                            $operation_model = new Operations();
                                            $operation_model->load_values($model);
                                            $operation_model->platform = 2;
                                            if (!$flag = $operation_model->save()) {
                                                $transaction->rollBack();
                                                $error .= ApiParser::parseErrors($operation_model->errors);
                    ...
                    if (!isset($application['application_no']) || empty($application['application_no'])) {
                        ...
                        if (isset($loan)) {
                            ...
                            $model->set_application_hierarchy();
                            ...
                            try {
                                if ($flag = $model->save()) {
                                    ...
                                    if (isset($application['fee']) && $application['fee'] != 0) {
                                        $operation_model = new Operations();
                                        $operation_model->load_values($model);
                                        $operation_model->platform = 2;
                                        if (!$flag = $operation_model->save()) {
    ...
    public function actionUpdate()
    {
        ...
        try {
            if ($flag = $model->save()) {
                ...
                if (isset($this->request['applications']['fee'])) {
                    $operation_model = Operations::findOne(['application_id' => $model->id, 'deleted' => 0]);
                    if (isset($operation_model)) {
                        if ($this->request['applications']['fee'] == 0) {
                            $operation_model->deleted = 1;
                            $operation_model->deleted_at = strtotime('now');
                            $operation_model->deleted_by = Yii::$app->user->getId();
                            $operation_model->save();
    ...
    public function actionVerificationupdate()
    {
        ...
        try {
            if ($flag = $model->save()) {
                /*if(isset($model->businessAppraisal))
                {
                    $business_appraisal_model = AppraisalsBusiness::findOne(['id' => $model->businessAppraisal->id]);
                    /*if($model->activity_id == 0)
                    {
                        $business_appraisal_model->deleted = 1;

                    } else {
                    //$business_appraisal_model->business_type = $model->activity_id;
                    //}
                    $business_appraisal_model->save();
                }*/
                if (isset($this->request['applications']['fee'])) {
                    $operation_model = Operations::findOne(['application_id' => $model->id, 'deleted' => 0]);
                    if (isset($operation_model)) {
    ...
    public function actionVerification()
    {
        ...
        if (!empty($data['data'])) {
            ...
            foreach ($members_data as $app_data) {
                $social_appraisal = SocialAppraisal::find()->where(['application_id' => $app_data['application_id']])->one();
                $app_data['social_appraisal_latitude'] = isset($social_appraisal) ? $social_appraisal->latitude : 0;
                $app_data['social_appraisal_longitude'] = isset($social_appraisal) ? $social_appraisal->longitude : 0;
                $business_appraisal = AppraisalsBusiness::find()->where(['application_id' => $app_data['application_id']])->one();
                $app_data['business_appraisal_latitude'] = isset($business_appraisal) ? $business_appraisal->latitude : 0;
                $app_data['business_appraisal_longitude'] = isset($business_appraisal) ? $business_appraisal->longitude : 0;
                $member[] = $app_data;
            }
            $response = [
                'members' => $member,
    ...
    public function actionSyncrecommendationimages()
    {
        ...
        $model->save();
        /*$i = base64_encode("");
        $i = base64_encode(gzencode($i));*/
        /*print_r($i);
        die();*/
        $base64_decode_data = base64_decode($request['image']);
        $images_data = json_decode(gzuncompress($base64_decode_data));
        /*print_r(base64_decode($images_data[0]));
        die();*/
        foreach ($images_data as $data)
        {
            //$data = base64_decode($data);
            $image_data = [];
            $image_data['images_data'] = $data;
            $image_data['parent_id'] = $model->id;
            $image_data['parent_type'] = 'visits';
            $image_data['image_type'] = 'visit_'.rand(1,9);
            $image = ImageHelper::syncImage($image_data);
            if(!$image)
            {
                $flag = false;
            }
