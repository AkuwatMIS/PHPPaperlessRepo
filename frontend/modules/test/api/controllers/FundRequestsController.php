            ...
            try {
                if ($flag = $model->save()) {
                    ...
                    $loan_tranches = LoanTranches::find()
                        ...
                        ->andWhere(['loans.status' => "approved",'loan_tranches.status' => 1, 'loans.branch_id' => $this->request['branch_id'], 'loan_tranches.fund_request_id' => 0,'loan_tranches.disbursement_id' => 0])->all();
                ...
                if ($flag) {
                    ...
                    foreach ($loan_tranches as $loan_tranch) {
                        $action_model = LoanTranchesActions::findOne(['parent_id' => $loan_tranch->id, 'action' => 'fund_request']);
