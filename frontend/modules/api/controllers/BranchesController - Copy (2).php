...
class BranchesController extends RestController
{
    ...
    public function actionProjects()
    {
        ...
        if(empty($response)) {
            ...
            if (!empty($areas)) {
                ...
                $areas_list = StructureHelper::getStructureList('areas', 'id', $area_keys);

                $branches = Yii::$app->Permission->getBranchList(Yii::$app->controller->id, Yii::$app->controller->action->id, $this->rbac_type,$user->id);
                $keys = array_keys($branches);
                $branches_list_old = StructureHelper::getStructureList('branches', 'id', $keys);

                $branch_projects = StructureHelper::getStructureList('branch_projects', 'branch_id', $keys);

                $areas_response = [];
                foreach ($areas_list as $key => $area_list) {
                    $area = ApiParser::parseArea($area_list);
                    if (!empty($branches_list_old)) {
                        foreach ($branches_list_old as $l) {
                            if ($branches_list_old->area_id == $areas_list->id) {
                                $branches_list[] = $l;
                            }
                        }
                        $branch_response = [];
    ...
    protected function getTeams($branch_id){
        ...
        $teams = StructureHelper::getStructureList('teams','branch_id', $branch_id);
        $team_response = [];
    ...
    protected function getFields($team_id){
        $fields = StructureHelper::getStructureList('fields','team_id', $team_id);
        $field_response = [];
