<?php
namespace frontend\modules\api\behaviours;
    /**
...
class Apiauth extends AuthMethod
{
    ...
    public function beforeAction($action)
    {
        ...
        if(!isset($api_key)){
            ...
            $key = in_array($api_key,StructureHelper::getApiKey());
        ...
        if(!isset($version_code)){
            ...
            //$version = \common\models\Versions::find()->where(['type' => 'App Version'])->andWhere(['<=','version_no',$version_code])->one();
            $version = (StructureHelper::getAppVersion() <= $version_code);
        }
